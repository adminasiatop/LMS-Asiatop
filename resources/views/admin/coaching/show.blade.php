...
@section('content')
    <div class="container-xl">
        <div class="row">
            <div class="col-12">
                <x-card.card title="List Videos - {{ $cnc->name }}">
                    <x-card.card-list>
                        @forelse($videos as $i => $video)
                            <x-card.card-list-item title="{{ $video->name }}" eps="{{ $video->episode }}"
                                ...
                                <x-button.button-link class="dropdown-item" title="Edit "
                                    url="{{ route('admin.videos.edit', [$cnc->slug, $video->video_code]) }}"
                                    icon="edit" />
                        ...
                        @empty
                            <x-alert.alert-danger title="This Cnc don't have any video" subTitle="Create new video"
                                url="{{ route('admin.videos.create', $cnc->slug) }}" icon="play-circle" />
                        @endforelse
                    </x-card.card-list>
                </x-card.card>
            </div>
